...
function assertWritablePath(filePath) {
    ...
    if (!normalised.startsWith(prefix + "/") && normalised !== prefix) {
        throw new Error(`Write rejected: path "${filePath}" is outside the allowed subfolder "${MYNA_SUBFOLDER}/". ` +
 ...
 */
server.tool("search", "Vault-wide full-text search using Obsidian's index. Returns file paths and matching lines.", { query: z.string().describe("Search term") }, async ({ query }) => {
 ...
 */
server.tool("search_content", "Search within note content. Returns matching content snippets.", { query: z.string().describe("Search term") }, async ({ query }) => {
 ...
 */
server.tool("read", "Read a note's full content or a specific section by heading. Specify either file (note name) or path (vault-relative path).", {
 ...
 */
server.tool("daily_read", "Read today's daily note content.", {}, async () => {
 ...
 */
server.tool("tasks", "List or query tasks via the Obsidian Tasks plugin. Optionally filter by query.", { query: z.string().optional().describe("Tasks plugin filter query (e.g. 'not done, due before today')") }, async ({ query }) => {
    const args = ["tasks"];
    if (query)
 ...
 */
server.tool("tags", "List all vault tags or find files with a specific tag.", { tag: z.string().optional().describe("Specific tag to search for (e.g. '#project')") }, async ({ tag }) => {
    const args = ["tags"];
    if (tag)
        args.push(`tag=${tag}`);
    args.push(...vaultArgs(), "--json");
 ...
 */
server.tool("backlinks", "Find all notes that link to a given file.", { file: z.string().describe("Note name to find backlinks for") }, async ({ file }) => {
 ...
 */
server.tool("property_read", "Read YAML frontmatter properties from a note.", { file: z.string().describe("Note name to read properties from") }, async ({ file }) => {
 ...
 */
server.tool("eval", "Run a Dataview query or JavaScript expression against the vault.", { expression: z.string().describe("JavaScript or Dataview query to evaluate") }, async ({ expression }) => {
 ...
 */
server.tool("create", "Create a new note with content. Fails if file already exists. Path must be under the myna/ subfolder.", {
 ...
 */
server.tool("append", "Append content to a note, optionally after a specific heading. Path must be under the myna/ subfolder.", {
 ...
 */
server.tool("prepend", "Prepend content to a note, optionally under a specific heading. Path must be under the myna/ subfolder.", {
    file: z.string().describe("Vault-relative path to the target note"),
 ...
 */
server.tool("daily_append", "Append content to today's daily note.", { content: z.string().describe("Content to append to the daily note") }, async ({ content }) => {
 ...
 */
server.tool("daily_prepend", "Prepend content to today's daily note.", { content: z.string().describe("Content to prepend to the daily note") }, async ({ content }) => {
 ...
 */
server.tool("overwrite_section", "Replace a section's content by heading. For structured metadata sections only. Path must be under the myna/ subfolder.", {
    file: z.string().describe("Vault-relative path to the target note"),
 ...
 */
server.tool("move", "Move or rename a file within the vault. Both source and destination must be under the myna/ subfolder.", {
 ...
 */
server.tool("delete", "Delete a file from the vault. Restricted to the myna/ subfolder.", { file: z.string().describe("Vault-relative path of the file to delete") }, async ({ file }) => {
    assertWritablePath(file);
 ...
 */
server.tool("create_from_template", "Create a note from a template file, substituting variables. Path must be under the myna/ subfolder.", {
 ...
 */
server.tool("property_set", "Set a YAML frontmatter property on a note. Path must be under the myna/ subfolder.", {
    file: z.string().describe("Vault-relative path to the target note"),
