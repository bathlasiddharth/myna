...
**Decision:** Phase 2 (Install) is a shell script that: (1) generates a project-level CLAUDE.md that loads main.md, steering files, and provides the skill directory for Claude Code, (2) registers the obsidian-cli MCP server with Claude Code via `claude mcp add`, (3) creates the `myna/` vault folder structure in the user's Obsidian vault, (4) copies `.example` config files to `_system/config/`. No adapter packaging, no format translation, no tool-specific syntax generation. The install script remains user-involved (not autonomous) per the original D035 intent. Supersedes Phase 2 scope from D035.
...
**Date:** 2026-04-07
**Context:** Myna was designed with Kiro CLI as the v1 target runtime (D035). During Phase 1 (Build), it became clear that Claude Code is a better fit: (a) Claude Code has native MCP support — Myna's obsidian-cli MCP server works directly via `claude mcp add`, (b) Claude Code uses CLAUDE.md as its entry point, which maps naturally to Myna's main agent instructions, (c) Claude Code has a large and growing user base, (d) the project is already being built with Claude Code, so the team has deep familiarity with its capabilities and constraints.
