...
Per-file structure (frontmatter, sections, entry formats) lives in the canonical schema directory: `docs/design/file-formats/`. Skills read these files at `~/.claude/myna/file-formats/` when they need to create or read structured vault files.

**Schema files:**
- `_conventions.md` — wiki-link convention, tolerance principle, read principle; load alongside any domain file
- `entities.md` — Project File, Person File, Team File
- `meetings.md` — Meeting File 1:1, Recurring, Adhoc
- `journal.md` — Daily Note, Weekly Note, Monthly Note, Contributions Log
- `drafts.md` — Draft File
- `queues.md` — Review Queue Entry, Review Inbox Entry
- `system.md` — Parked Context, Source File, Processed Channels Log, Central Link Index, Unified Dashboard, Learnings File

**Why a separate directory:** foundations.md is design rationale; `docs/design/file-formats/` is operational structure. Separating them lets skills point at stable anchors for specific file types without carrying the full doc in context, and lets the schema evolve without touching every rationale paragraph.

**Design rationale:**

*Project files* separate raw task storage from live view: `## Tasks` is where skills write task lines; `## Open Tasks` is a Dataview query that renders them. This keeps the Dataview block clean and lets skills append tasks without parsing or modifying a query block.

*Daily notes* use sync's structure (`## Morning Focus`, `## Sync — {HH:MM}` with `### Briefing` / `### Today's Meetings` / `### Tasks Due Today` / `### Dashboards`; `## End of Day — {HH:MM}` at the bottom). The `Morning Focus` section is always user-editable; sync never overwrites it. End-of-Day is bottom-anchored — it's a single-day flow, not a log. Re-run sync snapshots are prepended above the previous snapshot; previous snapshots are never touched.

*Weekly notes* include the start-of-week date in the `## Weekly Summary — {YYYY-MM-DD}` heading so multiple summary runs on the same weekly note are distinguishable.

*Meeting files* (1:1, Recurring) accumulate sessions newest-first — each new `## {YYYY-MM-DD} Session` block is prepended at the top, pushing older sessions down. This keeps the most recent session immediately visible without scrolling.

*Wiki-links* use the readable alias form: `[[Sarah Chen]]` not `[[sarah-chen]]`. Filenames are lowercase-hyphenated slugs; the human-readable display name lives in frontmatter `aliases:`. Files that need wiki-linking (Project, Person, Team, Meeting 1:1 and Recurring) must set `aliases:` at creation time.

*Log sections* (Timeline, Observations, Recognition, Contributions Log, Team Timeline, Source File, Meeting sessions) use newest-first prepend. The one exception is daily note End-of-Day, which stays bottom-anchored.

*Log entry format* is content-first: `- {content} [{provenance}] ({source-type}, {identity}, {YYYY-MM-DD})`. The old `[{date} | {source}]` prefix is retired. Date and source appear at the end, after the content the user actually cares about.
